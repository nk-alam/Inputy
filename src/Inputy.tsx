import React, { useState, ChangeEvent, FC, InputHTMLAttributes, useEffect } from 'react';
import './styles.css';
...
const Inputy: FC<InputyProps> = ({
  ...
  // Only render value on client side to prevent hydration mismatch
  const inputValueProp = isClient ? { value: inputValue } : {};

  return (
    <div className={`inputy-form-group ${wrapperClassName}`}>
      {label && (
        <label 
          className={`inputy-label ${labelClassName}`} 
          htmlFor={name}
        >
          {label}
          {required && (
            <span className={`inputy-mandatory ${mandatoryClassName}`}>
              *
            </span>
          )}
        </label>
      ...
      <input
        ...
        {...inputValueProp}
        id={name}
        className={`inputy-form-control ${inputClassName} ${className} ${
          focused ? 'inputy-focused' : ''
        } ${errorMessage ? 'inputy-error' : ''}`}
      ...
      {errorMessage && (
        <div 
          id={`${name}-error`} 
          className="inputy-error-message"
          role="alert"
        >
          {errorMessage}
        </div>
